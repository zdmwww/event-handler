...
public class CustomItemRepository implements ICustomItemRepository {
  ...
  public List<Item> findItemsByUsername(String username) {
    TypedQuery<Item> query =
        em.createQuery("select i from Item as i where i.creator = :username", Item.class);
  ...
  public List<Item> fetchItems(String consumerKey, int maxResult) {
    TypedQuery<Item> query =
        em.createQuery(
            "select i from Item as i where i.consumerKey = :consumerKey and i.status = :status",
            Item.class);
  ...
  public void cleanItems(int maxResult) {
    TypedQuery<Item> query =
        em.createQuery("select i from Item as i where i.status = :status", Item.class);
